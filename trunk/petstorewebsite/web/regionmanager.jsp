<%--
    Document   : regionmanager
    Created on : Dec 1, 2009, 2:51:49 PM
    Author     : Jeff
--%>

<%@page contentType="text/html" pageEncoding="UTF-8" import="entity.Region"%>
<%@page import="java.util.*" %>

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"
    "http://www.w3.org/TR/html4/loose.dtd">
<jsp:useBean id="region" class="entity.Region" scope="request" />
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>Pet Store Region Manager</title>
        ...
        <div id="wrap">
            ...
            <div class="center_content">
                <div class="left_content">
                    <div class="display">
                    <h2>Pet Store Region Manager</h2>
                    <a href="index.jsp">Back to Index</a><BR>

                            <form action="/PetStore/regionadmin" name="viewallregion" method="POST">
                                <input type="hidden" name="mode" value="<%=viewmode%>" />
                                <input type="hidden" name="viewcondition" value="<%=viewall%>" />
                                <input type="submit" value="View All Region" name="viewall" />&nbsp;&nbsp;
                            </form>

                            <form action="regionmanager.jsp" name="addnew" method="POST">
                                <input type="hidden" name="mode" value="<%=addnewmode%>" />
                                <input type="submit" value="Add Region" name="addnew" />&nbsp;&nbsp;
                            </form>

                            <form action="/PetStore/regionadmin" name="editregion" method="POST">
                                <input type="hidden" name="mode" value="<%=viewmode%>" />
                                <input type="hidden" name="edit" value="true" />
                                <input type="hidden" name="viewcondition" value="<%=viewbyCondition%>" />
                                Region ID: <input type="text" name="regionid" value="" size="10" />
                                <input type="submit" value="Edit" name="editregion" />&nbsp;&nbsp;
                            </form>
                            ...
                            <form action="/PetStore/regionadmin" name="edititem" method="POST">
                                <input type="hidden" name="mode" value="<%=editmode%>" />
                                Region Information<BR>
                                Region ID:<input type="text" name="regionid" size="20" value="<%=reg.getId()%>" /><BR>
                                Name: <input type="text" name="name" value="<%=reg.getName()%>" size="20" /><BR>
                                Manager: <input type="text" name="managerid" value="<%=reg.getManagerid()%>" size="20" /><BR>
                                <input type="submit" value="Edit Record" name="submit" />
                            </form>

                            <form action="/PetStore/regionadmin" name="deleteitem" method="POST">
                                <input type="hidden" name="mode" value="<%=deletemode%>" />
                                <input type="hidden" name="regionid" value="<%=reg.getId()%>" />
                            ...
                            %>
                            <Br><BR>
                            Enter New Region Record:
                            <form action="/PetStore/regionadmin" name="newinventoryrecord" method="POST">
                                <input type="hidden" name="mode" value="<%=addnewmode%>" />
                                Region Name:<input type="text" name="name" value="" size="20" />
                                Manager ID:<input type="text" name="managerid" value="" size="20" />
                                <input type="submit" value="Add" name="addnew" />
                            </form>
                            <% }
                                       if (mode != null && mode.equals(viewmode)) {
                                           ArrayList<Region> reglist = (ArrayList<Region>) request.getAttribute("regionlist");
                            %>
